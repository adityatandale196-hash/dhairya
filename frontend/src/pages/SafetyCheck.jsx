...
function ContactList({ contacts, message }) {
    ...
    return (
        <div>
            {contacts.map((c) => (
                <div className="sc-contact" key={c.contactId}>
                    <div>
                        <div className="sc-contact-name">{c.name}</div>
                        <div className="sc-contact-phone">
                            {c.phone}
                            {c.relationship ? " · " + c.relationship : ""}
                        </div>
                    </div>

                    <div className="sc-contact-buttons">
                        <a className="sc-whatsapp" href={whatsappLink(c.phone, message)} target="_blank" rel="noreferrer">WhatsApp</a>
                        <a className="sc-sms" href={smsLink(c.phone, message)}>SMS</a>
                        <a className="sc-tel" href={"tel:" + c.phone}>Call</a>
                    </div>
                </div>
            ))}
        </div>
...
function SafetyCheck() {
    ...
    // One timer that drives every waiting screen
    useEffect(() => {
    ...
    // Vibrate when a question needs an answer
    useEffect(() => {
    ...
    // Keep the screen awake where the browser supports it
    useEffect(() => {
        let lock = null;
        let cancelled = false;
    ...
    async function startCheck() {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setWarning(res.data.message || "The check could not be saved.");
            }
        } catch {
    ...
    async function escalate(level) {
        ...
        if (alertRecord) {
            try {
                await apiRequest(
                    "PUT",
                    "/api/alerts/" + alertRecord.alertId + "/escalate?userId=" + user.userId
                );
            } catch {
                // the on-screen alert still works
            }
        }

        if (level === 1) {
            goTo("alert1", intervalSeconds);
        } else {
            setPhase("alert2");
        }
    }
    ...
    const message2 =
        "URGENT: " + user.name + " has not confirmed being safe after repeated safety checks. " +
        ...
        return (
            <div className="sc-page sc-page-alert">
                <div className="sc-card">
                    ...
                    <h2>Your contacts</h2>
                    <ContactList contacts={contacts} message={message1} />
        ...
        return (
            <div className="sc-page sc-page-alert">
                <div className="sc-card">
                    <h1>🚨 Urgent alert</h1>
                    <p className="sc-text">
                        No confirmation of safety. Alert your contacts and call emergency
                        services now.
                    </p>
                    ...
                    <h2>Your contacts</h2>
                    <ContactList contacts={contacts} message={message2} />
